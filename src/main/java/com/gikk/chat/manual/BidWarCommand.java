...
public class BidWarCommand extends AbstractChatCommand
{
	...
	public boolean performCommand(String command, TwitchUser sender, String content, List<Emote> emotes)
	{
		...
		{
			Chat.GET().broadcast(sender, "Could not parse a valid number from "
		...
		{
			Chat.GET().broadcast(sender, "You can't afford your bid of " + bid
				+ " with your current "
	...
	private void tick()
	{
		...
		if (highestBid == null)
		{
			...
			{
				future.cancel(false);
				Chat.GET().broadcast("No bids were entered in 45 seconds. "
		...
		if (currentSecond == 5)
		{
			Chat.GET().broadcast(bid + " " + currency + " from "
				+ leader.getDisplayName() + "!"
				+ " Going once! PogChamp");
		}
		else if (currentSecond == 10)
		{
			Chat.GET().broadcast("Gooooing twice! PogChamp PogChamp PogChamp");
		}
		else if (currentSecond == 15)
		{
			future.cancel(false);
			Chat.GET().broadcast("SOLD!!! FeelsRareMan With a winning bid of "
				+ bid + " " + currency + ", "
