...
public abstract class AbstractChatCommand
{
	...
	public final void act(String command, TwitchUser sender, String content, List<Emote> emotes)
	{
		...
		for (ICondition condition : conditions)
		{
			...
			{
				condition.getResponse(command, sender).ifPresent(Chat.GET()::broadcast);
				return;
			}
		}
	 ...
	 * is seen that matches the commandPattern.<p>
	 *
	 ...
	 * @return {@code true} if the command performed its duty, {@code false} if not
	 */
